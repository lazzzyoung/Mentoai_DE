    ...
    try:
        ...
        if content_area:
            ...
            if job_desc_header:
                ...
                if job_desc_body:
                    extracted_data["job_description"] = clean_space(
                        job_desc_body.get_text()
                    )
            ...
            for table in tables:
                ...
                for row in rows:
                    ...
                    h_txt, b_txt = (
                        clean_space(th.get_text()),
                        clean_space(td.get_text()),
                    )
                    if not b_txt or any(
                        x in b_txt for x in ["관계없음", "비희망", "해당없음"]
                    ):
                        continue
                    ...
                    if any(kw in h_txt for kw in ["경력조건", "학력", "모집직종"]):
                        ...
                        kw in h_txt
                        for kw in ["우대조건", "전공", "자격면허", "외국어", "컴퓨터"]
                    ):
