...
def run_daily_producer():
    ...
    for i, row in enumerate(job_rows):
        try:
            ...
            # 상세 정보 (수집 실패 시에도 None이 아니라 기본값이 옴)
            detail = get_detail_info(auth_no)
            
            if not detail: # scraper 내부 에러로 None이 온 경우
