...
# 목적:
# - nginx 없이 host 443 포트를 ai-server:8000으로 직접 바인딩
...
echo "[0/7] 과거 리다이렉트 규칙 정리(컨테이너 outbound 443 장애 방지)"
...
del_iptables_rule_if_exists nat PREROUTING -p tcp --dport 443 -j REDIRECT --to-ports 8000
del_iptables_rule_if_exists nat OUTPUT -p tcp -o lo --dport 443 -j REDIRECT --to-ports 8000
...
echo "[6/7] iptables 영속화(리다이렉트 규칙은 정리된 상태 저장)"
...
echo "주의2) 외부 인바운드는 443(필수), 80(인증서용)만 권장"
